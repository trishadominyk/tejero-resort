...
class SlideDeckLens {
	...
	function copy( $slug, $new_lens_name = "", $new_lens_slug = "", $replace_js = false ) {
		...
		if( $original_lens ) {
			...
			if( is_writable( SLIDEDECK_CUSTOM_LENS_DIR ) ) {
				$destination_dir = SLIDEDECK_CUSTOM_LENS_DIR . "/" . $new_slug;
			...
			foreach( (array) $lens_files as $lens_file ) {
				...
				switch( $file_ext ) {
                    ...
                    case "php":
                        $old_classname = slidedeck_get_classname_from_filename( $original_lens['slug'] );
                        $new_classname = slidedeck_get_classname_from_filename( $new_slug );
    ...
    function copy_inc( $slug = "", $inc = 0 ) {
        if( is_dir( SLIDEDECK_DIRNAME . '/lenses/' . $slug ) || is_dir( SLIDEDECK_CUSTOM_LENS_DIR . '/' . $slug ) ) {
            $inc++;
    ...
    function get( $slug = "" ) {
        ...
        $lenses = wp_cache_get( $cache_key, slidedeck_cache_group( 'lenses-get' ) );
        if( $lenses == false ) {
            $lenses = array();
            ...
            $lens_files = (array) glob( SLIDEDECK_DIRNAME . '/lenses/' . $folders . '/lens.json' );
            ...
            if( is_dir( SLIDEDECK_CUSTOM_LENS_DIR ) ) {
                ...
                if( is_readable( SLIDEDECK_CUSTOM_LENS_DIR ) ) {
                    ...
                    $custom_lens_files = (array) glob( SLIDEDECK_CUSTOM_LENS_DIR . '/' . $folders . '/lens.json' );
            ...
            wp_cache_set( $cache_key, $lenses, slidedeck_cache_group( 'lenses-get' ) );
        }
    ...
    function get_css( $lens ) {
        $version = isset( $lens['meta']['version'] ) && !empty( $lens['meta']['version'] ) ? $lens['meta']['version'] : SLIDEDECK_VERSION;
		...
		if( isset( $lens['lens_mobile'] ) && !empty( $lens['lens_mobile'] ) ) {
            $lens_css_tags .= '<link rel="stylesheet" type="text/css" href="' . $lens['lens_mobile'] . '?v=' . $version . '" media="screen" />';
        }
		if( isset( $lens['lens_tablet'] ) && !empty( $lens['lens_tablet'] ) ) {
            $lens_css_tags .= '<link rel="stylesheet" type="text/css" href="' . $lens['lens_tablet'] . '?v=' . $version . '" media="screen" />';
        }
    ...
    function get_meta( $filename ) {
        ...
        $lens = wp_cache_get( $cache_key, slidedeck_cache_group( 'lenses-get-meta' ) );
        if( $lens == false ) {
	    ...
	    //To add external sources from addons. - Niranjan
	    $lens = apply_filters( "{$this->namespace}_add_external_source", $lens);
	    ...
	    // TODO: Loop Through sources
            ...
            wp_cache_set( $cache_key, $lens, slidedeck_cache_group( 'lenses-get-meta' ) );
        }
    ...
    function is_protected( $lens_filename ) {
        ...
        if( $file_exists ) {
            if( str_replace( "\\", "/", dirname( dirname( $lens_filename ) ) ) == str_replace( "\\", "/", SLIDEDECK_CUSTOM_LENS_DIR ) ) {
                $protected = false;
            }
        }
    ...
    function process_template( $nodes = array(), $slidedeck ) {
        ...
        if( isset( $nodes['created_at'] ) && !empty( $nodes['created_at'] ) ) {
		    ...
		    $nodes['created_at'] = human_time_diff( $nodes['created_at'], current_time( 'timestamp', 1 ) ) . __( " ago", $this->namespace );                    
                ...
                case "european":
                    $nodes['created_at'] = date( "jS F, Y", $nodes['created_at'] );
                break;
        ...
        if( isset( $video_meta ) ){ 
            $video_container = "<div id=\"video__{$video_meta['id']}__{$slidedeck['id']}-{$deck_iteration}-{$slide_counter}\" class=\"{$video_meta['service']} video-container\" data-video-id=\"{$video_meta['id']}\">";

if($video_meta['service'] == "tumblr"){
$video_container .= "<iframe id='tumblriFrame-video__{$video_meta['id']}__{$slidedeck['id']}-{$deck_iteration}' src='{$video_meta['permalink']}' width='100%' height='100%' frameborder='0'></iframe>";
}
$video_container .= "</div>";
        }
