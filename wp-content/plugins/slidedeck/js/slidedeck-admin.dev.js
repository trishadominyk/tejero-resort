...
/*
Code for toggle between custom image and button in case of CTA.
*/
jQuery("#options-cta-custom-opt-cbutton").click(function(){
     
	jQuery(".cbutton-parent").show();
        jQuery(".cimage-parent").hide();
     
});
jQuery("#options-cta-custom-opt-cimage").click(function(){
 	
	 jQuery(".cbutton-parent").hide();
         jQuery(".cimage-parent").show();
	
});
var mediaUploader;
jQuery("#cta-upload-img-id").click(function(e){
    ...
    mediaUploader.on('select', function() {
      attachment = mediaUploader.state().get('selection').first().toJSON();
       ...
       jQuery('#cta-preview').html( '<img src="'+attachment.url+'" alt="" style ="width:200px; height:200px" / >' ); 
       jQuery("#options-cta-custom-img-url").val(attachment.url);
    });
...
(function($){
        ...
        insertSlideDecks: function(){
            ...
            for(var d in datas){
                ...
                if(data.name == "slidedecks[]"){
                    shortcodes.push("[SlideDeck id=" + data.value + ( parent.slideDeck2iframeByDefault == true ? " iframe=1" : "" ) + "]");
                }
            }
     ...
     * Interaction scripting for copying lenses and deleting lenses
     */
    SlideDeckPlugin.SourceManagement = {
        elems: {},
        ...
        initialize: function(){
            ...
            this.elems.lensList = $('#slidedeck-sources');
     ...
     * Interaction scripting for copying lenses and deleting lenses
     */
    SlideDeckPlugin.TemplateManagement = {
        elems: {},
        ...
        initialize: function(){
            ...
            this.elems.lensList = $('#slidedeck-templates');
    ...
    SlideDeckPlugin.SourceManager = {
        ...
        initialize: function(){
		 ...
		 $('body').delegate('a.slidedeck-template-modal', 'click', function(event){
    ...
    $(document).ready(function(){
        ...
        SlideDeckPlugin.AddonsManagement.initialize();
        // Source Management Page
        SlideDeckPlugin.SourceManagement.initialize();
        // Source Management Page
        SlideDeckPlugin.TemplateManagement.initialize();
        ...
        // Addon key Verification Ajax
        if( $('#slidedeck-addon-key').length ){
            
            $('.sd-free-addon-form').delegate('#slidedeck-addon-key-submit', 'click', function(event){
                event.preventDefault();
                $.ajax({
                    url: ajaxurl + '?action=slidedeck_verify_addon_key&verify_addon_nonce=' + $('#verify_addon_nonce').val() + '&key=' + $('#slidedeck-addon-key').val(),
                    success: function( response ){
                        $('#sd-addon-register-msg').html(response);
                        $('#sd-addon-register-msg').css( {"display":"block"} );
                    }
                });
            });
		...
		if(this.value == "fullwidth"){
                    $('#slidedeck-fullwidth-dimensions').addClass('selected').animate({
                        height: 32,
                        opacity: 1
                    }, 500, function(){
                        SlideDeckPlugin.OptionsNav.resize();
                    ...
                    $('#slidedeck-fullwidth-dimensions').removeClass('selected').animate({
                        height: 0,
                        opacity: 0
                    }, 500, function(){
                        SlideDeckPlugin.OptionsNav.resize();
        ...
        /*
         * Slider Search request handel by ajax
         */
        $('#slidedeck-search-submit').bind('click', function(e){
            e.preventDefault();

            $('#slidedeck-table-sort-select').trigger('change');
        });
        ...
        $('.wp-submenu a[href$="slidedeck.php/support"], #slidedeck-support-questions a[href$="slidedeck.php/support"]').addClass('support-modal').attr('rel', 'get-support');
...
jQuery(document).ready(function(){
	...
	if( !jQuery('#instagram_client_id').val() ) {
	...
	jQuery.ajax({
	url: ajaxurl,
	data: 'action=slidedeck_save_instagram_code&access_token='+token,
	dataType: 'JSON',
	type: 'POST',
	success:function(data){
...
jQuery("#create_new_slider").click(function(){
	
	str = 'action=create_new_slider_temp';	
	jQuery.ajax({
	url:ajaxurl,
	data: str,
	type: 'POST',
	success:function(data){
		...
		jQuery("#create-new-slider-dynamic-slidedeck").remove();
		jQuery("#create-new-slider-template-slidedeck").remove();
		jQuery("#extra-new-slide-div").html(data);
...
function sdhtmlvideo(val)
{
	...
	if(val=='sdhtml5')
	{
		...
		jQuery('#youtubetext').val('');
		jQuery('#video-url-input').hide();	
		
	}
	else
	{
		jQuery(".slide-content-fields").show();
		jQuery(".uploadsdvideo").hide();
		jQuery(".change-media-src").show();
		jQuery('#video-url-input').show();	
	}
...
(function($, window, undefined){
     ...
     */
    SlideDeckPlugin.CustomCSSEditor = {
        textarea: null,
        
        initialize: function(){
            var self = this;
            
            self.textarea = $('#custom-slidedeck-css').find('textarea');
            if(self.textarea.length){
                this.editor = CodeMirror.fromTextArea(self.textarea[0], {
                    lineNumbers: true,
                    mode: "css",
                    theme: "slidedeck",
                    readOnly: false,
                    ...
                    onCursorActivity: function(cm) {
                        cm.save();
                        SlideDeckPlugin.CustomCSSEditor.editor.setLineClass(SlideDeckPlugin.CustomCSSEditor.line, null);
                        SlideDeckPlugin.CustomCSSEditor.line = SlideDeckPlugin.CustomCSSEditor.editor.setLineClass(SlideDeckPlugin.CustomCSSEditor.editor.getCursor().line, "activeline");
                    },
                    onChange: function(cm) {
                        if (self.textarea.sliderTimer)
                            clearTimeout(self.textarea.sliderTimer);
                        
                        // Set delay timer so a check isn't done on every single key stroke
                        self.textarea.sliderTimer = setTimeout(function(){
                            SlideDeckPreview.ajaxUpdate();
                        }, 990 ); // Save after 990ms of inactivity
                    }
                });
    ...
    $(document).ready(function(){
        ...
        SlideDeckPlugin.CustomCSSEditor.initialize();
    });
})(jQuery, window, null);