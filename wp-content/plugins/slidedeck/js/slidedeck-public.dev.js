...
(function($){
    ...
    window.SlideDeckVideoAPIs = function( slidedeck ){
        ...
        deckElement.find('.cover .play').click(function(event){
            ...
            if( cover.siblings('.video-container').data('video-id') ){
                videoContainerID = cover.siblings('.video-container').data('video-id');
            }else{
            		
                videoContainerID = cover.siblings('.video-container').attr('id').split('__')[1];
            }
            ...
            // check for new lens
            var newlens = false;
            if( jQuery(deckElement).parent('.slidedeck-frame').hasClass('lens-prime') || jQuery(deckElement).parent('.slidedeck-frame').hasClass('lens-parfocal') ){
                var slides = deckElement.find('li');
                newlens = true;
            } else {
            ...
            // change selector for fashion lense

            if( jQuery(slidedeckFrame).hasClass('lens-fashion') || newlens){
                videoContainerID = cover.siblings('.video-container').attr('id');
                var videoIndex = slides.index( jQuery( '#'+videoContainerID  ) .closest('li') );
            } else {
        ...
        this.videoEnded = function( index, playerType, deckId ){
            ...
            if( advanceAtVideoEnd ){
                ...
                deckContext.next(function(){
                    var current = deckContext.current;
                    var currentSlide = $( '#' + deckId ).find('.slide:eq(' + ( current - 1 ) + ')');

                    // More vertical accommodations.
                    if( __isVerticalDeck( deckElement ) ){
                        current += 1;
                        currentSlide = $( '#' + deckId ).find('.slidesVertical .slide:eq(' + ( current - 1 ) + ')');
                    }

                    var playerType = deckElement.find('.slide .video-container:eq(' + ( current - 1 ) + ')')[0].className.split(' ')[0];
                    ...
                    if( autoStartNextVideo ){
                        currentSlide.find('a.play-video-button').parents('.slide').addClass('sd2-hide-slide-content');
        ...
        this.loadYouTubeVideo = function(deckElement, slideIndex){
            ...
            if( jQuery(slidedeckFrame).hasClass('lens-fashion') || jQuery(slidedeckFrame).hasClass('lens-prime') || jQuery(slidedeckFrame).hasClass('lens-parfocal') ){
                var videoContainer = deckElement.find('li.slide:eq(' + ( slideIndex ) + ') .video-container.youtube');
            } else {
                var videoContainer = deckElement.find('.slide:eq(' + ( slideIndex ) + ') .video-container.youtube');
            }
            ...
            if( jQuery(slidedeckFrame).hasClass('lens-fashion') || jQuery(slidedeckFrame).hasClass('lens-prime') || jQuery(slidedeckFrame).hasClass('lens-parfocal') ){
                var videoIndex = deckElement.find('li').index( jQuery( '#' + videoContainerID ).parents('li') ) - ( __isVerticalDeck( deckElement ) ? 1 : 0 );
            } else {
            ...
            addEvent( "onStateChange", iFrameYouTubePlayer, function(video){
                switch( video.data ){
                    case 0:
                        // Video Ended...
        ...
        this.loadVimeoVideo = function(deckElement, slideIndex){
            ...
            if( jQuery(slidedeckFrame).hasClass('lens-fashion') || jQuery(slidedeckFrame).hasClass('lens-prime') || jQuery(slidedeckFrame).hasClass('lens-parfocal') ){
                var videoContainer = deckElement.find('li:eq(' + ( slideIndex ) + ') .video-container.vimeo');
            } else {
                var videoContainer = deckElement.find('.slide:eq(' + ( slideIndex ) + ') .video-container.vimeo');
            }
            ...
            // Added https to fixed cross domain origin.
            jQuery(videoContainer).append( '<iframe id="vimeoiFrame-' + videoContainerID + '" src="https://player.vimeo.com/video/' + videoID + '?api=1&byline=0&title=0&portrait=0&player_id=vimeoiFrame-' + videoContainerID + '" width="100%" height="100%" frameborder="0"></iframe>' );

            //var videoIndex = slides.index( jQuery( '#' + videoContainerID ).closest('.slide') );
            if( jQuery(slidedeckFrame).hasClass('lens-fashion') || jQuery(slidedeckFrame).hasClass('lens-prime') || jQuery(slidedeckFrame).hasClass('lens-parfocal') ){
                var videoIndex = deckElement.find('li').index( jQuery( '#' + videoContainerID ).parents('li') ) - ( __isVerticalDeck( deckElement ) ? 1 : 0 );
            } else {
                var videoIndex = deckElement.find('dd').index( jQuery( '#' + videoContainerID ).parents('dd') ) - ( __isVerticalDeck( deckElement ) ? 1 : 0 );
        ...
        this.loadDailyMotionVideo = function(deckElement, slideIndex){
            ...
            if( jQuery(slidedeckFrame).hasClass('lens-fashion') || jQuery(slidedeckFrame).hasClass('lens-prime') || jQuery(slidedeckFrame).hasClass('lens-parfocal') ){
                var videoContainer = deckElement.find('li:eq(' + ( slideIndex ) + ') .video-container.dailymotion');
            } else {
                var videoContainer = deckElement.find('.slide:eq(' + ( slideIndex ) + ') .video-container.dailymotion');
            }
            ...
            addEvent( "ended", dailymotionPlayer, function(e){
            ...
            addEvent( "playing", dailymotionPlayer, function(e){
                jQuery(deckElement[0]).parents('.slidedeck-frame').addClass("sd2-video-playing");
                // Pause Autoplay on the SlideDeck when the video API says it's playing.
                jQuery(deckElement[0]).slidedeck().pauseAutoPlay = true;
            });
	...
	if( this.elems.container.find('.slidedeck-overlays-wrapper').find('.overlay_pinterest').val() == 1 )
	{	
        
        $('.slidedeck-overlays-wrapper a:last-child').html('Pin It');
        $('.slidedeck-overlays-wrapper a:last-child').addClass('slidedeck-overlay slidedeck-overlay-type-pinit');
}
        //Calculate overlays width becuase pin it is added here.
        self.overlayWidth = 0;
        this.elems.container.find(".slidedeck-overlays-wrapper a.slidedeck-overlay").each(function(ind){
            self.overlayWidth += $(this).outerWidth();
        });

        this.elems.container.width(self.overlayWidth );
    };
            ...
            if( slidedeck_ie ){
                if( slidedeck_ie <= 8.0 ){
                    $slidedeck.find('.sd2-slide-background').each(function(){
                        ...
                        var $slide = $slideBackground.closest('.slide');
 ...
 *
 * Copyright (C) 2012 hbwsl, http://www.dtelepathy.com
 *
...
(function($){
    ...
    SlideDeckCover.prototype.initializeBack = function(){
        ...
        this.elems.restart.bind('click', function(event){
            event.preventDefault();
            jQuery('.slidedeck-cover-back').hide();
            self.revert();
        });
    ...
    $(document).ready(function(){
        ...
        $('.slidedeck-frame img').bind('contextmenu', function(e){
