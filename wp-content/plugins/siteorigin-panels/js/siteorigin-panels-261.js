...
module.exports = Backbone.View.extend( {
	...
	attach: function ( options ) {
		...
		if( ! this.supports( 'addWidget' ) ) {
			...
			welcomeMessageContainer.find('.so-widget-add').hide();
		}
		if( ! this.supports( 'addRow' ) ) {
			toolbar.find('.so-row-add' ).hide();
			welcomeMessageContainer.find('.so-row-add').hide();
		}
		if( ! this.supports( 'prebuilt' ) ) {
			toolbar.find('.so-prebuilt-add' ).hide();
			welcomeMessageContainer.find('.so-prebuilt-add').hide();
		}

		if ( ! ( this.supports( 'addWidget' ) || this.supports( 'addRow' ) || this.supports( 'prebuilt' ) ) ) {
			welcomeMessageContainer.find('.so-message-wrapper').html(panelsOptions.loc.welcomeMessage.addingDisabled);
		}
